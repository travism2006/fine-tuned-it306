...
public class DialManageVehicles extends JDialog
{
		...
		editCarBttn.addActionListener(new ActionListener()
		{
			...
			public void actionPerformed(ActionEvent e)
			{
				...
				if(!hold.equals(""))
				{
					...
					JPanel editPanel = makeEditPanel(car);
			...
			/**Returns a jpanel that contains the editible fields per car type*/
			private JPanel makeEditPanel(Vehicle car)
			{
				...
				//input fields for panel
				makeInput = new JTextField(car.getMake(),VMSPro_Constants.pixelColWidth);
				modelInput = new JTextField(car.getModel(),VMSPro_Constants.pixelColWidth);
				colorInput = new JComboBox<>(CarColors.values());
				colorInput.setSelectedItem(car.getColor());
				
				if(car.getCustomer() == null)
				{custInput = new JTextField("0",VMSPro_Constants.pixelColWidth);}
				else
				{custInput = new JTextField(
						String.format("%d",car.getCustomer().getCustomerId()),
						VMSPro_Constants.pixelColWidth);}
	...
	private Vehicle editSedanPnl(JPanel somePanel, Sedan someCar)
	{
		...
		//grab all fields and make sure they are non-empty
		checkEditCarPanel(somePanel, someCar);
		
		//see if new customer id # is in the system
		try{
