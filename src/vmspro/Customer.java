...
public class Customer implements VMSPro_Standard_Behavior {
   ...
   * 
   * @param someFName
   *          the first name to be assgined for this field
   * @param someLName
   *          the last name for this customer
   */
   ...
   *          the last name for this customer
   * @param someAddress
   *          the associated address for this customer
   */
   ...
   * 
   * @param someFName
   *          the first name to be given to this customer.
   ...
   * 
   * @param someLName
   *          the last name to be given to this customer
   * @return whether or not assignment is successful
   */
   ...
   * 
   * @param someAddress
   *          the primary address to be given to this customer
   * @return whether or not assignment is successful
   */
   ...
   * 
   * @param car
   *          the car title to be added to this customer's list
  ...
  /**
   * Uses the HashMap's clear() property to render the map empty. Requires no
   * validation.
   */
  ...
  public String toString() {
    String out = String.format("Customer ID: %d, Full Name: %s %s, Primary Address: %s", this.custID, this.firstName,
        this.lastName, this.address.toString());
    out += "\n";
    for (Vehicle car : carListMap.values()) {
      if (car instanceof Sedan) {
        Sedan sidsSedan = (Sedan) car;
        out += String.format("Lot#: %d, Car VIN: %s, Car Type: Sedan,", sidsSedan.getLotNumber(), sidsSedan.getVin());
      } else if (car instanceof Truck) {
        Truck tammysTruck = (Truck) car;
        out += String.format("Lot#: %d, Car VIN: %s, Car Type: Truck,", tammysTruck.getLotNumber(),
            tammysTruck.getVin());
      } else if (car instanceof Van) {
        Van vincentsVan = (Van) car;
        out += String.format("Lot#: %d, Car VIN: %s, Car Type: Van,", vincentsVan.getLotNumber(), vincentsVan.getVin());
      }
    }
    return out;
  }
  ...
  /**
   * Compares on the customer ID field on this object and the given object. If
   * the IDs are identical, then it is said that the given object is essentially
   * the same customer as the current one.
   * 
   * @param someCust
   *          the customer object to compare against
   * @return whether or not the given and current customer's IDs are the same
   */
  public boolean equals(Customer someCust) {
    if (this.custID == someCust.getCustomerId())
      return true;
    return false;
  }
}