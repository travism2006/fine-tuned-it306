...
public class DialManageVehicles extends JDialog {
  ...
  private void makeMainPanel(VMSPro app) {
    ...
    addCarBttn.addActionListener(new ActionListener() {
      ...
      public void actionPerformed(ActionEvent e) {
        ...
        String ans = String.valueOf(JOptionPane.showInputDialog(contentPane, "What type of vehicle:",
            "Add Vehicle Type", JOptionPane.OK_CANCEL_OPTION, null, opts, opts[0]));
        ...
        case "SEDAN":
          ...
          if (car != null) {
            ...
            if (custID == -2 || custID == -1) {
              String line = String.format("\n%d\t%s\t%s\t%s\t%d\t%s\t%s\tN\\A", lotid, vin, make, model, year, color,
                  "SEDAN") + "\n";
              totalCarArea.append(line);
            } else {
              String line = String.format("\n%d\t%s\t%s\t%s\t%d\t%s\t%s\t%d", lotid, vin, make, model, year, color,
                  "SEDAN", custID) + "\n";
              totalCarArea.append(line);
            }
          }
        ...
        case "VAN":
          ...
          if (car != null) {
            ...
            if (custID == -2 || custID == -1) {
              String line = String.format(VMSPro_Constants.HasNoClient, lotid, vin, make, model, year, color, "VAN")
                  + "\n";
              totalCarArea.append("\n" + line);
            } else {
              String line = String.format(VMSPro_Constants.HasClient, lotid, vin, make, model, year, color, "VAN",
                  custID) + "\n";
              totalCarArea.append("\n" + line);
            }
          }
    ...
    remvCarBttn.addActionListener(new ActionListener() {
      ...
      public void actionPerformed(ActionEvent e) {
        ...
        if (!hold.equals("")) {
          ...
          String ans = String.valueOf(JOptionPane.showInputDialog(contentPane, "What type of vehicle:", "Edit Vehicle",
              JOptionPane.OK_CANCEL_OPTION, null, opts, opts[0]));
          Vehicle car = sysApp.getVehicleByVIN(ans);
          sysApp.removeCarX(car);
          JOptionPane.showMessageDialog(contentPane, "The vehicle with VIN " + car.getVin() + " has been removed");

          // now show empty string where the car's string was in the
          // jtextarea
          String dat = DialManageVehicles.getTxtArea().getText();
          if (car.getCustomer() == null) {
            dat.replace(DialManageVehicles.doAddLineNoClient(car.getLotNumber(), car.getVin(), car.getMake(),
                car.getModel(), car.getYear(), car.getColor()), "");
          } else {
            dat.replace(DialManageVehicles.doAddLineWithClient(car.getLotNumber(), car.getVin(), car.getMake(),
                car.getModel(), car.getYear(), car.getColor(), car.getCustomer().getCustomerId()), "");
          }
    ...
    // pseudo-table
    totalCarArea = new JTextArea(VMSPro_Constants.TxtCarFullHeader, VMSPro_Constants.TxtAreaHeight,
        VMSPro_Constants.TxtAreaWidth);
  ...
  private void fillTable(JTextArea someArea, VMSPro app) {
    ...
    while (carIT.hasNext()) {
      ...
      if (car != null) {
        int lotid = car.getLotNumber(), year = car.getYear(), custID = -2;
        if (car.getCustomer() == null)
          custID = -1;
        else if (car.getCustomer().equals(null))
          custID = -1;
        ...
        String make = car.getMake(), model = car.getModel(), vin = car.getVin();
        CarColors color = car.getColor();
        if (car instanceof Sedan) {
          if (custID == -2 || custID == -1)
            line += String.format("\n%d\t%s\t\t%s\t%s\t%d\t%s\t%s\tN\\A", lotid, vin, make, model, year, color, "SEDAN")
                + "\n";
          else {
            line += String.format("\n%d\t%s\t\t%s\t%s\t%d\t%s\t%s\tN\\A", lotid, vin, make, model, year, color, "SEDAN",
                custID) + "\n";
          ...
          if (custID == -2 || custID == -1) {
            line += String.format("\n%d\t%s\t\t%s\t%s\t%d\t%s\t%s\tN\\A", lotid, vin, make, model, year, color, "TRUCK")
                + "\n";
          } else {
            line += String.format("\n%d\t%s\t\t%s\t%s\t%d\t%s\t%s\t%d", lotid, vin, make, model, year, color, "TRUCK",
                custID) + "\n";
          }
        } else if (car instanceof Van) {
          if (custID == -2 || custID == -1) {
            line += String.format("\n%d\t%s\t\t%s\t%s\t%d\t%s\t%s\tN\\A", lotid, vin, make, model, year, color, "VAN")
                + "\n";
          } else {
            line += String.format("\n%d\t%s\t\t%s\t%s\t%d\t%s\t%s\t%d", lotid, vin, make, model, year, color, "VAN",
                custID) + "\n";
          }
        }
      }
  ...
  private Vehicle makeAndAddSedan(JPanel somePanel) {
    ...
    if (checkVehicleInput(somePanel)) {
      ...
      Customer custX;
      try {
        custX = sysApp.getCustomerByID(Integer.parseInt(custInput.getText()));
      } catch (NumberFormatException e) {
        custX = null;
      }
      boolean mayConvert = Boolean.parseBoolean((String) convertibleSedan.getSelectedItem());
      Vehicle newSedan;
  ...
  private Vehicle makeAndAddTruck(JPanel somePanel) {
    ...
    somePanel.add(towlbl);
    somePanel.add(towInput);
  ...
  /**
   * Functions as a conditional-continuous method for validating and making
   * vehicle objects.
   * 
   * @param somePanel
   *          the panel to refer to that is the parent for any JOP
   */
  private boolean checkVehicleInput(JPanel somePanel) {
    int create = -1;
    do {
      create = JOptionPane.showConfirmDialog(null, somePanel, "Add Vehicle and Type", JOptionPane.OK_CANCEL_OPTION);
      if (create == JOptionPane.OK_OPTION) {
        // Check the VIN
        boolean goodVIN = VMSPro.checkVIN(vinInput.getText());
        while (!goodVIN) {/*
                           * if invalid VIN, check if the user wants to try
                           * again
                           */
          int resp = JOptionPane.showConfirmDialog(somePanel, "Bad VIN input, do you want to try again?",
              "Invalid VIN Entered", JOptionPane.YES_NO_OPTION);
          if (resp == JOptionPane.YES_OPTION) {
            create = JOptionPane.showConfirmDialog(null, somePanel, "Add Vehicle and Type",
        ...
        while (!goodMake) {/*
                            * if invalid car make, check if the user wants to
                            * try again
                            */
          int resp = JOptionPane.showConfirmDialog(somePanel, "Bad Manufacturer input, do you want to try again?",
              "Invalid Make Entered", JOptionPane.YES_NO_OPTION);
          if (resp == JOptionPane.YES_OPTION) {
            create = JOptionPane.showConfirmDialog(null, somePanel, "Add Vehicle and Type",
                JOptionPane.OK_CANCEL_OPTION);
        ...
        while (!goodYear) {/*
                            * if invalid car year, check if the user wants to
                            * try again
                            */
          int resp = JOptionPane.showConfirmDialog(somePanel, "Bad year input, do you want to try again?",
              "Invalid Year Entered", JOptionPane.YES_NO_OPTION);
          if (resp == JOptionPane.YES_OPTION) {
            create = JOptionPane.showConfirmDialog(null, somePanel, "Add Vehicle and Type",
                JOptionPane.OK_CANCEL_OPTION);
          } else if (resp == JOptionPane.NO_OPTION) {
  ...
  /**
   * Returns the txt area in the dialog for accessing and filling of the primary
  ...
  public static String doAddLineWithClient(int lotid, String vin, String make, String model, int year, CarColors color,
      int custID) {
    String line = String.format(VMSPro_Constants.HasClient, lotid, vin, make, model, year, color, "VAN", custID) + "\n";
    return line;
    // totalCarArea.append("\n"+line);
  }
}