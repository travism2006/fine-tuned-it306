...
public class DialManageVehicles extends JDialog {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                case "SEDAN":
                    ...
                    if (car != null) {
                        ...
                        if (custID == -2 || custID == -1) {
                            String line = String.format("\n%d\t%s\t%s\t%s\t%d\t%s\t%s\tN\\A", lotid, vin, make, model,
                                    year, color, "SEDAN") + "\n";
                            totalCarArea.append(line);
                        } else {
                            String line = String.format("\n%d\t%s\t%s\t%s\t%d\t%s\t%s\t%d", lotid, vin, make, model,
                                    year, color, "SEDAN", custID) + "\n";
                            totalCarArea.append(line);
                        }
                    }
                    break;
                ...
                case "VAN":
                    ...
                    if (car != null) {
                        ...
                        if (custID == -2 || custID == -1) {
                            String line = String.format(VMSPro_Constants.HasNoClient, lotid, vin, make, model, year,
                                    color, "VAN") + "\n";
                            totalCarArea.append("\n" + line);
                        } else {
                            String line = String.format(VMSPro_Constants.HasClient, lotid, vin, make, model, year,
                                    color, "VAN", custID) + "\n";
                            totalCarArea.append("\n" + line);
                        }
                    }
        ...
        remvCarBttn.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if (!hold.equals("")) {
                    ...
                    String ans = String.valueOf(JOptionPane.showInputDialog(contentPane, "What type of vehicle:",
                            "Edit Vehicle", JOptionPane.OK_CANCEL_OPTION, null, opts, opts[0]));
                    ...
                    if (car.getCustomer() == null) {
                        dat.replace(DialManageVehicles.doAddLineNoClient(car.getLotNumber(), car.getVin(),
                                car.getMake(), car.getModel(), car.getYear(), car.getColor()), "");
                    } else {
                        dat.replace(DialManageVehicles.doAddLineWithClient(car.getLotNumber(), car.getVin(),
                                car.getMake(), car.getModel(), car.getYear(), car.getColor(),
        ...
        while (carIT.hasNext()) {
            ...
            if (car != null) {
                ...
                if (car instanceof Sedan) {
                    if (custID == -2 || custID == -1)
                        line += String.format("\n%d\t%s\t\t%s\t%s\t%d\t%s\t%s\tN\\A", lotid, vin, make, model, year,
                                color, "SEDAN") + "\n";
                    else {
                        line += String.format("\n%d\t%s\t\t%s\t%s\t%d\t%s\t%s\tN\\A", lotid, vin, make, model, year,
                                color, "SEDAN", custID) + "\n";
                    ...
                    if (custID == -2 || custID == -1) {
                        ...
                        line += String.format("\n%d\t%s\t\t%s\t%s\t%d\t%s\t%s\t%d", lotid, vin, make, model, year,
                                color, "TRUCK", custID) + "\n";
                    }
    ...
    private boolean checkVehicleInput(JPanel somePanel) {
        ...
        do {
            ...
            if (create == JOptionPane.OK_OPTION) {
                ...
                while (!goodVIN) {/*
                                   * if invalid VIN, check if the user wants to
                                   * try again
                                   */
                ...
                while (!goodMake) {/*
                                    * if invalid car make, check if the user
                                    * wants to try again
                                    */
                    int resp = JOptionPane.showConfirmDialog(somePanel,
                            "Bad Manufacturer input, do you want to try again?", "Invalid Make Entered",
                ...
                while (!goodYear) {/*
                                    * if invalid car year, check if the user
                                    * wants to try again
                                    */
    ...
    public static String doAddLineNoClient(int lotid, String vin, String make, String model, int year,
            CarColors color) {
    ...
    public static String doAddLineWithClient(int lotid, String vin, String make, String model, int year,
            CarColors color, int custID) {
