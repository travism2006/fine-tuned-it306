...
/**
 * For VMS Pro, Java's Swing and AWT features are used. This is the umbrella
 * class that manages each data type that the application will process. Uses
 * Java SE Collections interfaces including <code>Set</code> enabling the use of
 * <code>Iterator</code> to be easily implemented upon general use of the API.
 * 
 * @verion 1.0.b
 * @author tmitchu2
 * @author hammar
...
public final class VMSPro {
  ...
  /**
   * Default constructor for initializing lists and stacks for application
   * starting.
   */
  public VMSPro() {
  ...
  /**
   * Adds a customer as long as it is a valid customer object. Checks for null
   * object input.
   * 
   * @param someCust
   *          the customer obj to be added to the list
   * @return the success of adding the customer to the list
   */
  public boolean addCustomer(Customer someCust) {
    if (someCust == null)
      return false;
    else if (this.getCustomerList().contains(someCust))
      return false;
   ...
   * 
   * @param someCar
   *          the car to be added to the car list
   * @return the success of adding the car to the car list
   */
  public boolean addCar(Vehicle someCar) {
    if (someCar == null)
      return false;
    else if (this.getVINList().contains(someCar.getVin()))
      return false;
    return this.listCars.add(someCar);
  }

  /**
   * Remove the given customer as long as they are already in the system. Upon
   * removal from list, a stack grabs the removed customer object.
   * 
   * @param someCust
   *          the customer to be removed
   * @param keepCars
   *          determines if the cars associated are also deleted
   * @return whether or not the operation was a success
   */
  public boolean removeCustomerX(Customer someCust, boolean keepCars) {
    if (someCust == null)
      return false;
    else if (!this.listCust.contains(someCust))
      return false;
    ...
    if (listCust.remove(someCust)) {
      ...
      while (itCars.hasNext()) {
        ...
        /*
         * ------------------------------------------------------------ If
         * keepCars is true, then the dealership <b>does not</b> delete the
         * cars. If false, the cars are deleted in addition to the customer.
         * ----------------------------------------------------------
         */
        if (keepCars == false) {
  ...
  /**
   * Remove the given vehicle as long as it is already in the system. Upon
   * removal from list, a stack grabs the removed vehicle object.
   * 
   * @param someCar
   *          the customer to be removed
   * @return whether or not the operation was a success
  ...
  public boolean removeCarX(Vehicle someCar) {
    if (someCar == null)
      return false;
    else if (!this.listCars.contains(someCar))
      return false;
   ...
   * empty. Upon successful removal, the customer is pushed onto the stack that
   * tracks all deleted customers.
   * 
   * @param keepCars
   *          determines if linked cars are deleted or not
   ...
   * empty. Upon successful removal, the car is pushed onto the stack that
   * tracks all deleted vehicles.
   * 
   * @return the success or failure of removal
   */
  ...
  /**
   * Removes the very last vehicle in the list as long as the list is not empty.
   * Upon successful removal, the car is pushed onto the stack that tracks all
   * deleted vehicles.
   * 
   * @return the success or failure of removal
   */
  ...
  /**
   * Grabs all <code>toString()</code> information from the customer object and
   * adds additional system meta data about it.
   * 
   * @param someCust
   *          the customer obj to extract <code>toString()</code> from
  ...
  public String getCustomerXReport(Customer someCust) {
    if (someCust == null)
      ...
      String toStr = String.format("Position in VMS Pro: %d\n\nCustomer Details:\n%s", counter, someCust.toString());
      return toStr;
  ...
  /**
   * Grabs all <code>toString()</code> information from the vehicle object and
   * adds additional system meta data about it.
   * 
   * @param someCar
   *          the vehicle obj to extract <code>toString()</code> from
  ...
  public String carXReport(Vehicle someCar) {
    if (someCar == null)
      ...
      if (someCar instanceof Sedan) {
        toStr = String.format("Position in VMS Pro: %d\n\nVehicle(Sedan) Details:\n%s", counter,
            ((Sedan) someCar).toString());
      } else if (someCar instanceof Truck) {
        toStr = String.format("Position in VMS Pro: %d\n\nVehicle(Truck) Details:\n%s", counter,
            ((Truck) someCar).toString());
      } else if (someCar instanceof Van) {
        toStr = String.format("Position in VMS Pro: %d\n\nVehicle(Van) Details:\n%s", counter,
            ((Van) someCar).toString());
      }
      return toStr;
  ...
  /**
   * Gets all vehicles with the specified color and makes a general summary of
   * this color.
   * 
  ...
  public String carColorXReport(CarColors someColor) {
    ...
    while (coloredCars.hasNext()) {
      Vehicle car = coloredCars.next();
      String vin = car.getVin(), make = car.getMake(), model = car.getModel();
      int year = car.getYear();
      if (car instanceof Sedan) {
        out += String.format("Type: Sedan\nVIN: %s, Make: %s, Model: %s, Year: %d", vin, make, model, year);
      } else if (car instanceof Truck) {
        out += String.format("Type: Truck\nVIN: %s, Make: %s, Model: %s, Year: %d", vin, make, model, year);
      } else if (car instanceof Van) {
        out += String.format("Type: Van\nVIN: %s, Make: %s, Model: %s, Year: %d", vin, make, model, year);
      }
    }
    return out;
  }
  ...
  /**
   * Returns the first customer that has a matching id. Primarily used by GUI to
   * identify customers against input jtextfields.
   * 
   * @param someID
   *          the id to check with
   * @return the matching customer object or null
   */
  public Customer getCustomerByID(int someID) {
    if (someID < VMSPro_Constants.MIN_NUMBER_INPUT)
      return null;
  ...
  /**
   * Returns the first vehicle that has a matching VIN. Primarily used by GUI to
   * identify cars against input jtextfields.
   * 
   * @param someVIN
   *          the VIN to check with
   * @return the matching vehicle object or null
   */
  public Vehicle getVehicleByVIN(String someVIN) {
    if (!VMSPro.checkString(someVIN))
      return null;

    // otherwise valid VIN was given
    Iterator<Vehicle> carItr = listCars.iterator();
    while (carItr.hasNext()) {
      Vehicle car = carItr.next();
      if (car.getVin().equals(someVIN))
        return car;
    }
    return null;
  }
   ...
   * <i>The use of <code>Set</code> is not used due to lack of support for each
   * vehicle instance that has a specific make value since it is possible for
   * one 'make' value to be seen several times across different vehicle
   * instances.</i>
   * 
  ...
  /**
   * Returns a <code>List</code> of the currently existing customers' first name
   * within the VMS Pro appication.
   * 
  ...
  /**
   * Returns a <code>List</code> of the currently existing customers' last name
   * within the VMS Pro appication.
   * 
   ...
   * 
   * @param typeX
   *          the type of vehicle to report on
  ...
  /**
   * Goes through the list of customrs and tries to find the matching customer
   * with the given first and last name. If the list is empty, bad input is
   * passed, returns null.
   * 
   * @param fname
   *          the first name to check against
   * @param lname
   *          the last name to check against
  ...
  public Customer findCustomerByName(String fname, String lname) {
    if (!VMSPro.checkString(fname))
      return null;
    else if (!VMSPro.checkString(lname))
      return null;
    Iterator<Customer> custIT = listCust.iterator();
    while (custIT.hasNext()) {
  ...
  /**
   * Determines if the string provided points to null value in memory, is
   * containing null value, or is equivalent to an empty string. Passing these 3
   * tests, the string is said to be 'valid' and 'unique'.
   * 
   * @param someStr
   *          the string to be tested for validity and uniqueness
  ...
  /**
   * Determines if the passed argument is valid and has the required length to
   * be a correct VIN.
   * 
   * @param someStr
   *          the string to be checked against
   * @return whether or not the given str is a valid VIN
   */
  public static boolean checkVIN(String someStr) {
    if (!checkString(someStr))
      return false;
   ...
   * 
   * @param text
   *          the str to be validated and parsed
   * @return whether or not the input str is strictly a number
   * @throws NumberFormatException
   *           if the input text cannot be parsed as int
   */
