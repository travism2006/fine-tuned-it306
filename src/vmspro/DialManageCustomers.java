...
/**
 * A JDialog instance that uses a main panel to show all client data as well as
 * details regarding ownerships and empty titles.
 * 
 * @author tmitchu2
 */
...
public class DialManageCustomers extends JDialog {
  ...
  private void makeMainPanel(VMSPro app) {
    ...
    addClientBttn.addActionListener(new ActionListener() {
      ...
      private JPanel makeAddCustomerPanel() {
        ...
        if (sysApp.getVehicleNoClient() != null && !sysApp.getVehicleNoClient().isEmpty()) {
          carVinInput = new JComboBox<String>((String[]) sysApp.getVehicleNoClient().toArray());
        }
        ...
        String[] opts = { "" };
        JOptionPane.showOptionDialog(null, "", "title", JOptionPane.OK_CANCEL_OPTION, JOptionPane.INFORMATION_MESSAGE,
            null, opts, opts[0]);
    ...
    remvClientBttn.addActionListener(new ActionListener() {
      ...
      public void actionPerformed(ActionEvent e) {
        ...
        if (!hold.equals("")) {
          ...
          String ans = String.valueOf(JOptionPane.showInputDialog(contentPane, "What type of vehicle:", "Edit Customer",
          ...
          int keepCars = JOptionPane.showConfirmDialog(returnDialog(e),
              "Delete all associated vehicles with this customer?", "Remove Customer Vehicles",
    ...
    totalCustomerArea = new JTextArea(VMSPro_Constants.TxtCustomerHeader, VMSPro_Constants.TxtAreaHeight,
  ...
  /**
   * Given a panel, grabs all required data, validates fields, then creates and
   * returns a customer object. Behaves similar to the <i> Builder</i> design
   * pattern.
   */
  ...
  private boolean checkClientFields(JPanel panel) {
    int create = -1;
    do {
      create = JOptionPane.showConfirmDialog(null, panel, "Add Customer", JOptionPane.OK_CANCEL_OPTION);
  ...
  private void fillTable(JTextArea someArea, VMSPro app) {
    ...
    while (clientIT.hasNext()) {
      ...
      if (cust != null) {
        ...
        line += String.format(VMSPro_Constants.HasCar, id, firstN, lastN, carCount, adrs.toString()) + "\n";
      }
    }
